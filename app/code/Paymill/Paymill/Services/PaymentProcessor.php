...
class PaymentProcessor
{
     ...
     */
    public function __construct($privateKey = null, $apiUrl = null, $libBase = null, $params = null, LoggingInterface $loggingClassInstance = null)
    {
    ...
    {
        $this->_transactionsObject = new \Paymill\Paymill\Services\Transactions($this->_privateKey, $this->_apiUrl);
        $this->_preauthObject = new \Paymill\Paymill\Services\Preauthorizations($this->_privateKey, $this->_apiUrl);
        $this->_clientsObject = new \Paymill\Paymill\Services\Clients($this->_privateKey, $this->_apiUrl);
        $this->_paymentsObject = new \Paymill\Paymill\Services\Payments($this->_privateKey, $this->_apiUrl);
    }
    ...
    private function _validateResult($paymillObject, $type)
    {
        ...
        if (isset($paymillObject['data']['response_code']) && $paymillObject['data']['response_code'] !== 20000) {
            ...
            throw new \Paymill\Paymill\Services\Exception("Invalid Result Exception: Invalid ResponseCode", $paymillObject['data']['response_code']);
        }
        ...
        if (isset($paymillObject['response_code']) && $paymillObject['response_code'] !== 20000) {
            ...
            throw new \Paymill\Paymill\Services\Exception("Invalid Result Exception: Invalid ResponseCode", (int)$paymillObject['response_code']);
        }

        if (!isset($paymillObject['id']) && !isset($paymillObject['data']['id'])) {
            $this->_log("No $type created.", var_export($paymillObject, true));
            throw new \Paymill\Paymill\Services\Exception("Invalid Result Exception: Invalid Id");
        } else {
        ...
        if ($type == 'Transaction') {
            if (is_array($paymillObject) && array_key_exists('status', $paymillObject)) {
                if ($paymillObject['status'] == "closed") {
                    ...
                    throw new \Paymill\Paymill\Services\Exception("Invalid Result Exception: Invalid Orderstate");
                } else {
                    // another error occured
                    $this->_log("Unknown error." . var_export($paymillObject, true));
                    throw new \Paymill\Paymill\Services\Exception("Invalid Result Exception: Unknown Error");
                }
            } else {
                // another error occured
                $this->_log("$type could not be issued.", var_export($paymillObject, true));
                throw new \Paymill\Paymill\Services\Exception("Invalid Result Exception: $type could not be issued.");
            }
        } else {
            return true;
        }
     ...
     * @param any $logger
     */
    public function setLogger(LoggingInterface $logger = null)
    {
        $this->_logger = $logger;
    }
