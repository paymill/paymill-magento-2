define([], function() {
	...
	return {
		...
		validate : function(iban) {
			...
			var ibanWithoutCheckDigits = this.iban.substr(0, this.iban.length - 2);
			var ibanWithZeroCheckDigits = ibanWithoutCheckDigits + "00";
			var ibanCheckDigits = this.iban.substr(this.iban.length - 2, 2);

			var calcCheckDigits = (98 - this.calculate(ibanWithZeroCheckDigits))
					.toString();
		...
		changeCharacterPosition : function() {
			var firstFourCharacters = this.iban.substr(0, 4);
			var leftOverString = this.iban.substr(4);
		...
		calculate : function(iban) {
			...
			while (loop) {
				...
				var tempInt = remainder + iban.substr(start, length);
